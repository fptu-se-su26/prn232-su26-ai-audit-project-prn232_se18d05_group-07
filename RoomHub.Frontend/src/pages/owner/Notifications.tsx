  ...
  return (
    <div className="space-y-6 pb-12 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-on-surface">Trung tâm thông báo</h2>
          <p className="text-xs text-gray-500">Nơi nhận và phản hồi thông tin từ khách thuê trọ của bạn.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 soft-shadow p-12 text-center flex flex-col items-center justify-center min-h-[350px]">
          ...
          <h3 className="text-base font-bold text-on-surface mb-1">Không có thông báo mới</h3>
        ...
        <div className="space-y-3">
          {notifications.map((notif, index) => {
            const isResponse = notif.type === 'ContractResponse';
            return (
              <Reveal key={notif.id} delay={index * 50}>
                ...
                >
                  ...
                  <div className={`absolute top-0 bottom-0 left-0 w-1.5 ${
                    isResponse ? 'bg-green-500' : 'bg-blue-500'
                  }`}></div>

                  <div className="flex items-start gap-4">
                    {/* Icon Column */}
                    <div className={`w-10 h-10 rounded-xl shrink-0 flex items-center justify-center ${
                      isResponse ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-500'
                    }`}>
                      <span className="material-symbols-outlined text-[22px]">
                        {isResponse ? 'assignment_turned_in' : 'campaign'}
                      </span>
                    </div>
