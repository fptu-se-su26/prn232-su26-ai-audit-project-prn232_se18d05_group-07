...
const Subscription: React.FC = () => {
  ...
  const getPlanName = (plan: string) => {
    switch (plan) {
      ...
      case 'Monthly': return 'Pro (Theo Tháng)';
      case 'Yearly': return 'Pro (Theo Năm)';
  ...
  return (
    <div className="space-y-8">
      {/* Page Title block */}
      <div>
        <h2 className="text-xl font-bold text-on-surface">Gói dịch vụ & Hạn mức tài nguyên</h2>
        <p className="text-xs text-gray-500 mt-1">Theo dõi tài nguyên sử dụng và nâng cấp các gói dịch vụ cao cấp của RoomHub.</p>
      </div>
        ...
        <>
              ...
              <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-lg shadow-orange-500/20">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h2 className="text-2xl font-black mt-1">{subStatus ? getPlanName(subStatus.plan) : 'Starter (Free)'}</h2>
                  </div>
          ...
          <div className="space-y-4 pt-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h3 className="font-black text-on-surface text-base">Bảng so sánh các gói cước</h3>
                  ...
                  <div className={`rounded-3xl border-2 p-6 h-full flex flex-col justify-between transition-all hover:scale-[1.01] ${p.color}`}>
                    <div>
                      ...
                      <ul className="space-y-3 mb-8">
                        {p.perks.map((perk) => (
                          <li key={perk} className="text-xs flex items-start gap-2 text-left">
      ...
      {isUpgradeModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-[28px] max-w-2xl w-full p-6 md:p-8 soft-shadow relative border border-gray-100 overflow-y-auto max-h-[90vh]">
