...
public record DepositRequest(int HoldDurationDays, string PaymentMethod, string? TransactionId, long? PaymentProofId);
public record DepositPaymentProofDto(long Id, string OriginalFileName, DateTime ExpiresAt);
public record ViewingBookingDto(long Id, int RoomId, string RoomTitle, string TenantId, string TenantName,
    ...
    DateTime CreatedAt, DateTime UpdatedAt, DateTime? CompletedAt, DateTime? CancelledAt, decimal RequiredDepositAmount, DepositDto? Deposit);
