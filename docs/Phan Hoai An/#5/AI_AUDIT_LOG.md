...
- Đồng bộ hóa dữ liệu Tenant Dashboard (Dashboard.tsx) kết nối API động, xóa user card cứng ở Tenant Sidebar.
- Làm sạch layout login/register, ẩn Navbar/Footer và thêm nút Back to Home.
- Xóa user card cứng của chủ nhà ở Owner Sidebar, động hóa top bar và avatar dropdown.
- Tạo migrations EF Core (`AddPendingContractStatus`) và cập nhật database.
...
| Ngày sử dụng | 08/06/2026 |
| Công cụ AI | Antigravity |
| Mục đích sử dụng | Động hóa Tenant Dashboard & Sidebar và ẩn Navbar/Footer trang Auth |
| Phần việc liên quan | Frontend Tenant Layout / Tenant Dashboard / App.tsx |
| Mức độ sử dụng | Hỗ trợ nhiều |
...
AI định vị chính xác file Layout và Dashboard. Đề xuất:
- Loại bỏ code HTML hiển thị profile card cứng của tenant tại `TenantLayout.tsx`.
- Viết hàm `fetchRoom` sử dụng endpoint `/tenant/room` để động hóa toàn bộ stats (Phòng đang thuê, Tiền thuê, Trạng thái, Diện tích) thay cho dữ liệu seed.
- Sử dụng hook `useLocation` trong `App.tsx` kiểm tra nếu pathname là trang đăng nhập/đăng ký/quên mật khẩu thì ẩn Navbar và Footer.
- Viết thêm button "Quay lại trang chủ" trên header form của `Login.tsx` và `Register.tsx`.

#### 4.3. Minh chứng

- File liên quan: `TenantLayout.tsx`, `Dashboard.tsx`, `App.tsx`, `Login.tsx`, `Register.tsx`
...
| Ngày sử dụng | 08/06/2026 |
| Công cụ AI | Antigravity |
| Mục đích sử dụng | Sửa lỗi tên/email cứng trong layout Owner và cập nhật migrations EF Core database |
| Phần việc liên quan | Frontend Owner Layout / Backend EF Migration |
| Mức độ sử dụng | Hỗ trợ nhiều |
...
AI gợi ý:
- Tích hợp `useAuth` hook vào `OwnerLayout.tsx` để thay thế thông tin cứng Phan Hoài An / owner@roomhub.vn tại top bar và dropdown.
- Xóa bỏ block user card trong Sidebar của `OwnerLayout.tsx`.
- Sử dụng terminal đề xuất lệnh EF migrations `dotnet ef migrations add AddPendingContractStatus` và `dotnet ef database update` để đồng bộ CSDL SQL Server với code domain.

#### 4.3. Minh chứng

- File liên quan: `OwnerLayout.tsx`, backend Migration files
- Lệnh chạy: `dotnet ef migrations add AddPendingContractStatus` và `dotnet ef database update`
...
- Kiểm tra các câu lệnh EF migrations (`AddPendingContractStatus`) và cập nhật database (`Done`).
- Đăng nhập tài khoản Chủ nhà / Khách thuê → Xác minh sidebar đã dọn dẹp card cứng, thông tin ở góc trên bên phải hiển thị đúng tên/email thật của user đăng nhập động.
- Truy cập login/register → Kiểm tra Navbar và Footer biến mất, nút "Quay lại trang chủ" hoạt động trơn tru.
