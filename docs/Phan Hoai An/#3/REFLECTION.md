...
```text
Phần logic validation sức chứa tối đa mặc định của AI chỉ là hiển thị một dòng thông báo alert tĩnh đơn giản và khóa nút bấm lưu. Em tự nhận thấy điều này chưa tối ưu cho trải nghiệm người dùng vận hành, vì vậy em đã tự cải tiến thành một block thông báo màu đỏ nhấp nháy kèm biểu tượng Material Symbol warning rực rỡ nằm ngay dưới ô nhập liệu khi số lượng người lớn hơn 2. Điều này giúp giao diện trở nên vô cùng bắt mắt, thu hút sự chú ý của chủ trọ và tăng chất lượng trải nghiệm UX của hệ thống. Đồng thời, trong trang InvoiceList.tsx, mã nguồn do AI viết ban đầu thiếu khai báo hai biến state phục vụ phân trang là `currentPageNum` và `pageSize` dẫn đến lỗi build. Em đã chủ động khai báo bổ sung các biến state này giúp mã nguồn biên dịch hoàn toàn sạch sẽ, đạt 0 lỗi TypeScript và 0 cảnh báo. Ngoài ra, em tự tinh chỉnh định vị route khớp Hash Router cho `#/owner/invoices/create` trước `#/owner/invoices` trong App.tsx để triệt tiêu lỗi khớp nhầm chuỗi con, và cập nhật activeMatches trong OwnerLayout.tsx để giữ menu sáng đúng vị trí. Em cũng thiết lập định vị trích xuất ID động cho route `#/owner/invoices/:id` đảm bảo an toàn sau trang chốt tiền và cập nhật menu activeMatches tương ứng.
```
...
|---|:---:|---|
| Ghi nhận việc dùng AI trung thực | 5 | Khai báo chi tiết các lần sử dụng AI |
