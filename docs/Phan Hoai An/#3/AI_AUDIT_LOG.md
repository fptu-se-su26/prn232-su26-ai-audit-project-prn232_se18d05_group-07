...
- Xây dựng trang quản lý hóa đơn & tài chính `InvoiceList.tsx` gồm 5 thẻ summary cards, 4 quick actions, view Table/Card, Bulk actions bar, và bộ 5 modals chốt nợ trọ chi tiết có cảnh báo tài khoản offline.
- Xây dựng biểu mẫu chốt tiền tháng hàng loạt 5 bước `InvoiceCreate.tsx` có bảng tính toán realtime 16 cột, phiên bản di động Card Accordion, Drawer xem breakdown chi tiết và bộ 6 Modals/Drawer tác vụ nghiệp vụ tài chính.
- Xây dựng trang chi tiết hóa đơn `InvoiceDetail.tsx` lồng ghép Preview bill A4 giống hóa đơn thật, breakdown chi tiết phí dịch vụ điện nước, lịch sử timeline đóng tiền, lưu ghi chú, và bộ 4 modals tác vụ nghiệp vụ tài chính (Export Excel, Record Payment, Send Notification nhắc offline, Cancel Invoice).
- Chạy kiểm thử thành công biên dịch đóng gói sản phẩm tĩnh bằng `npm run build` không có lỗi.

### Mô tả mục tiêu sử dụng AI

```text
Sử dụng trợ lý AI Antigravity để thiết kế và triển khai giao diện quản lý nghiệp vụ chuẩn SaaS cho vai trò Chủ nhà (Owner) trong RoomHub. AI hỗ trợ cài đặt layout, Sidebar, Topbar và cấu hình Hash Router thông minh trong App.tsx. AI viết mã nguồn React chi tiết cho PropertyList.tsx, PropertyDetail.tsx, UnitDetail.tsx, đặc biệt là cơ chế tương tác thời gian thực Live Grid Generator trong PropertyCreate.tsx tự động sinh sơ đồ tầng. AI hỗ trợ thiết kế ListingCreate.tsx và trang danh sách quản lý tin trọ ListingList.tsx (lọc đa năng, Table/Card views, Bulk actions bar và bộ 5 modals nghiệp vụ). AI cũng hỗ trợ viết logic validation sức chứa tối đa trong Add Tenant Modal, cấn trừ tiền cọc bàn giao trong End Tenancy và tích hợp sâu liên kết hai chiều toàn giao diện. Thêm vào đó, AI hỗ trợ xây dựng trang InvoiceList.tsx (5 summary cards, 4 quick actions, bulk actions bar và 5 modals), trang chốt tiền hàng loạt InvoiceCreate.tsx (stepper 5 bước, bảng 16 cột realtime, card accordion di động và 5 modals), và trang chi tiết hóa đơn InvoiceDetail.tsx (A4 bill preview, breakdown chỉ số cũ/mới, timeline thanh toán và 4 modals chốt nghiệp vụ).
```
...
| Ngày sử dụng | 30/05/2026 |
| Công cụ AI | Antigravity |
| Mục đích sử dụng | Xây dựng trang danh sách quản lý hóa đơn `InvoiceList.tsx` có 5 modals tài chính |
| Phần việc liên quan | Frontend / Invoice Management / Interactive Financial Modals |
| Mức độ sử dụng | Hỗ trợ nhiều / Sinh chính nội dung |

#### 4.1. Prompt đã sử dụng

```text
tiếp theo hãy thực hiện cập nhật bổ sung tiếp trang giao diện Chặng 3 (Tài chính & Hóa đơn): 7. Invoice Management (/owner/invoices). tôi sẽ cung cấp mổ tả chi tiết, và hiện tại bây giờ đang có lỗi ở file App.tsx ở dòng cụ thể là import ListingCreate... hãy thực hiện sửa chữa luôn...
```

#### 4.2. Kết quả AI gợi ý

AI đã sửa chữa thành công các lỗi import trùng lặp/thiếu trong `App.tsx` và xây dựng giao diện `InvoiceList.tsx` bóng bẩy theo phong cách SaaS tài chính. Hỗ trợ hiển thị 5 thẻ thống kê tài chính lớn, bộ lọc tìm kiếm nâng cao, tabs phân loại trạng thái thanh toán, view toggles và thanh Bulk Actions màu cam tự động trượt lên khi có phòng được tích chọn. Lập trình đầy đủ bộ 5 Modals chi tiết (Record Payment, Mark as Paid, Cancel Invoice, Export Excel, và Send Notification nhắc nợ).

#### 4.3. Phần sinh viên/nhóm đã sử dụng từ AI

Sử dụng toàn bộ mã nguồn của trang `InvoiceList.tsx` làm màn hình chính trong quản trị hóa đơn & công nợ của Chủ nhà.

#### 4.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

- Chạy lệnh kiểm thử biên dịch tsc tĩnh và phát hiện lỗi thiếu khai báo 2 biến state phục vụ phân trang: `currentPageNum` và `pageSize` dẫn đến build lỗi. Nhóm chủ động viết bổ sung hai biến state này vào đầu component giúp hệ thống chạy phân trang động cực kỳ mượt mà.

#### 4.5. Minh chứng

| Loại minh chứng | Nội dung |
|---|---|
| Link commit | [DE180303] feat: build owner invoice management page with finance cards and 5 modals |
| File liên quan | RoomHub.Frontend/src/pages/owner/InvoiceList.tsx |
...
| Ngày sử dụng | 30/05/2026 |
| Công cụ AI | Antigravity |
| Mục đích sử dụng | Xây dựng trang chốt tiền tháng hàng loạt `InvoiceCreate.tsx` có Stepper 5 bước và bảng tính realtime |
| Phần việc liên quan | Frontend / Bulk Invoicing Form / Real-time Calculations / Responsive Accordion Cards |
| Mức độ sử dụng | Hỗ trợ nhiều / Sinh chính nội dung |
...
AI gợi ý kế hoạch chi tiết xây dựng `InvoiceCreate.tsx` có cấu trúc 5 bước Stepper, card thiết lập phạm vi (chọn tài sản lọc phòng, tháng tính hóa đơn, hạn thanh toán và trạng thái nháp/chờ thu). Thiết lập bảng 16 cột với 5 dòng mẫu sinh động minh họa đầy đủ các kịch bản thực tế (Hợp lệ, Thiếu chỉ số, Lỗi chỉ số, Đã có hóa đơn) có khả năng nhập trực tiếp và tự động tính toán điện nước, phụ thu, chiết khấu và tổng tiền thời gian thực. AI cũng lập trình Drawer xem breakdown hóa đơn dự kiến, thanh Bulk Actions cam nhạt đặt phụ thu hàng loạt, Sticky Summary Panel tóm tắt tài chính và bộ 5 Modals bổ trợ (Xác nhận tạo, Lưu nháp, Xuất Excel, Gửi thông báo nhắc nợ). Ở bản mobile, AI vẽ các Cards gọn gàng có Accordion để dễ nhập liệu.

#### 4.3. Phần sinh viên/nhóm đã sử dụng từ AI

Sử dụng toàn bộ mã nguồn biểu mẫu Stepper 5 bước, bảng tính 16 cột realtime, Drawer và bộ 5 Modals trong tệp `InvoiceCreate.tsx`.

#### 4.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

- Tinh chỉnh các kiểm tra định tuyến Hash Router trong `App.tsx` để xử lý kiểm tra `#/owner/invoices/create` trước `#/owner/invoices`, tránh xung đột trùng khớp chuỗi con.
- Cấu hình lại activeMatches trong `OwnerLayout.tsx` để mục menu `"Hóa đơn & Chốt tiền"` luôn sáng màu cam đặc trưng khi người dùng đang thao tác chốt tiền.
- Chạy lệnh `npx tsc --noEmit` kiểm tra biên dịch đạt kết quả thành công sạch sẽ 100%, 0 lỗi TypeScript.

#### 4.5. Minh chứng

| Loại minh chứng | Nội dung |
|---|---|
| Link commit | [DE180303] feat: implement bulk monthly invoice form page with 5-step stepper and interactive calculations |
| File liên quan | RoomHub.Frontend/src/pages/owner/InvoiceCreate.tsx |
| Screenshot | ![alt text](image-5.png) |
| Kết quả chạy/test | Chạy dev server mượt mà, nhập chỉ số mới tự nhảy tổng tiền ngay lập tức, tsc build sạch sẽ |
...
| Ngày sử dụng | 30/05/2026 |
| Công cụ AI | Antigravity |
| Mục đích sử dụng | Xây dựng trang chi tiết hóa đơn `InvoiceDetail.tsx` và Tính năng xuất hóa đơn Excel |
| Phần việc liên quan | Frontend / Invoice Detail / Payment History / Export Excel Modals / Responsive Card |
| Mức độ sử dụng | Hỗ trợ nhiều / Sinh chính nội dung |
...
AI đã đề xuất cấu trúc giao diện 2 cột SaaS chuyên nghiệp. Lập trình thành công tệp `InvoiceDetail.tsx` hiển thị tiêu đề, trạng thái và các tóm tắt tài chính dynamic. Bản xem trước A4 hóa đơn (Bill Preview) giống tờ giấy thật (Logo RoomHub, bảng kê 8 dòng chi phí), breakdown chỉ số điện nước, timeline lịch sử thanh toán nợ trọ và notes audit logs. AI thiết kế bộ 4 Modals đầy đủ nghiệp vụ: Record Payment Modal cập nhật nợ trọ, Send Notification có cảnh báo Offline, Cancel Invoice Modal ghi lý do hủy, và đặc biệt là Export Excel Modal tích hợp 9 checkboxes, chọn kiểu mẫu và vẽ một **Spreadsheet Mockup Grid** mô phỏng bảng tính Excel thu nhỏ rất sinh động. Ở bản di động, giao diện chuyển đổi sang Card dọc bo tròn gọn gàng.

#### 4.3. Phần sinh viên/nhóm đã sử dụng từ AI

Sử dụng toàn bộ mã nguồn của trang `InvoiceDetail.tsx` làm giao diện quản trị chi tiết hóa đơn dịch vụ & xuất Excel chính cho Chủ nhà.

#### 4.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

- Tinh chỉnh cơ chế so khớp Hash Router trong `App.tsx` bằng hàm `startsWith` cho `#/owner/invoices/` và ưu tiên đặt sau case kiểm tra `/create` để triệt tiêu hoàn toàn lỗi re-route nhầm.
- Cập nhật menu `OwnerLayout.tsx` mục `"Hóa đơn & Chốt tiền"` thêm trường `activeMatches` chứa `'owner-invoice-detail'` giúp thanh Sidebar sáng đèn đúng mục.
- Chạy biên dịch `npx tsc --noEmit` đạt kết quả sạch sẽ 100% 0 lỗi TypeScript.

#### 4.5. Minh chứng

| Loại minh chứng | Nội dung |
|---|---|
| Link commit | [DE180303] feat: build owner invoice detail page with realistic A4 paper preview and 4 financial modals |
| File liên quan | RoomHub.Frontend/src/pages/owner/InvoiceDetail.tsx |
| Screenshot | ![alt text](image-6.png) |
| Kết quả chạy/test | Đóng gói Vite thành công tĩnh 100%, re-route và thanh toán mượt mà nhảy số realtime |
