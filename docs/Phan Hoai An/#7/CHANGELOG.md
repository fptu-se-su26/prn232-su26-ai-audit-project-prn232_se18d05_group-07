...
- [x] **Biên dịch Frontend**:
  - Chạy lệnh build thành công, loại bỏ tất cả các cảnh báo kiểu dữ liệu.

## Thay đổi chi tiết

| STT | Nội dung thay đổi | Người thực hiện | File/Module liên quan | Minh chứng |
|---:|---|---|---|---|
| 1 | Thêm state, hàm xóa và Custom Modal Confirm xóa thông báo cho Chủ nhà | Phan Hoài An | `pages/owner/Notifications.tsx` | UI hiển thị Custom Confirm modal & xóa thành công |
| 2 | Thay thế `window.confirm` bằng Custom Modal Confirm khi xóa thông báo ở Khách thuê | Phan Hoài An | `pages/tenant/Notifications.tsx` | Hộp thoại Custom Confirm modal thay thế hoàn toàn browser default |
| 3 | Thay thế `window.confirm` bằng Custom Modal Confirm khi từ chối nhận phòng ở Khách thuê | Phan Hoài An | `pages/tenant/Notifications.tsx` | Hộp thoại cảnh báo từ chối màu cam hoạt động tốt |
| 4 | Sửa nút Về trang chủ trong Avatar menu để làm sạch hash và định hướng đúng URL `/` | Phan Hoài An | `OwnerLayout.tsx`, `TenantLayout.tsx` | Điều hướng chuẩn và URL sạch hash |
| 5 | Tự động dọn dẹp hash trên URL khi bấm Logo hoặc danh mục điều hướng chính | Phan Hoài An | `Navbar.tsx` | Tránh bị đè dashboard khi điều hướng về các trang công cộng |
| 6 | Biên dịch kiểm thử TypeScript và build thành công | Phan Hoài An | Frontend | Build succeeded |
...
```text
AI trợ lý Antigravity đã hỗ trợ tích cực trong việc: (1) Thiết kế Custom Modal Confirm bằng React State & Tailwind CSS, tích hợp hoạt ảnh mượt mà cho các hành động xóa thông báo và từ chối nhận phòng; (2) Phân tích lỗi cấu trúc hash-routing dẫn đến hành vi hiển thị chồng lấn trang và đề xuất cách làm sạch hash trên Navbar và Layouts; (3) Tiến hành các thay đổi trực tiếp lên file code Frontend và xác minh biên dịch thành công.
```
